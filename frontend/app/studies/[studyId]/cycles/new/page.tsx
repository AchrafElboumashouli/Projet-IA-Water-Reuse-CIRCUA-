...
import { CyclePlantNamesForm, CyclePlantNames } from "@/components/CyclePlantNamesForm";
import { CycleTable } from "@/components/CycleTable";
import { api } from "@/lib/api";
import { buildEmptyRows } from "@/lib/constants";
import { DraftRow, StageRole } from "@/lib/types";
...
export default function NewCyclePage() {
  ...
  const [plantNames, setPlantNames] = useState<CyclePlantNames>({
    plant_1: "",
    plant_2: "",
    plant_3: "",
  });
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      // Everything travels in one POST: cycle metadata + this cycle's
      // plant names + all 24 raw rows. The backend computes
      // average/std/removal, persists cycle_results with NO plant
      // columns, and writes the plant names once into the separate
      // cycle_plants table (linked via cycle_id).
      const cycle = await api.createCycle({
        ...
        end_date: endDate,
        plants: {
          plant_1: plantNames.plant_1.trim(),
          plant_2: plantNames.plant_2.trim(),
          plant_3: plantNames.plant_3.trim(),
        },
        rows,
      });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Enter the three replicates for each parameter and stage. Average, STD,
          and removal update live as you type — Save recalculates and stores the
          final table.
        </p>
      </div>
      ...
      <CyclePlantNamesForm value={plantNames} onChange={setPlantNames} />
