...
function removal(
  ...
  if (wastewater === null || current === null || wastewater === 0) return null;
  return ((wastewater - current) / wastewater) * 100;
}
...
/**
 * Mirrors backend/app/crud.py::build_cycle_results for instant client-side
 * preview. The backend recomputes authoritatively on save — this is purely
 * so the table can show live numbers as the user types.
 */
export function computeAllRows(
  ...
  for (const row of rows) {
    ...
    if (row.stage !== ("Wastewater" as Stage)) {
      const baseline = byKey.get(`${row.parameter}|Wastewater`);
      if (baseline) {
