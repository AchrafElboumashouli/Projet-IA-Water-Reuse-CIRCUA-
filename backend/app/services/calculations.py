...
def compute_removal(wastewater_value: Number, current_value: Number) -> Optional[float]:
    """((Wastewater - Current) / Wastewater) * 100, guarding against div-by-zero."""
    if wastewater_value is None or current_value is None:
        return None
    if wastewater_value == 0:
        return None
    return ((wastewater_value - current_value) / wastewater_value) * 100.0
